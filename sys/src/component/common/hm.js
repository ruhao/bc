...
export default {
    ...
    methods: {
        ...
        getData() {
            this.$http.post(`http://` + this.getTest() + `/${this.apimodel}/list`, this.fliter).then(res => {
        ...
        remove(index) {
            this.$Modal.confirm({
                ...
                onOk: () => {
                    setTimeout(() => {
                        this.$http.delete(`http://` + this.getTest() + `/${this.apimodel}/data/` + this.fliter.data6[index]._id).then(res => {
                            //通过传入的id来删除需要删除的内容
        ...
        handleSubmit() {
            ...
            this.$http.post(`http://` + this.getTest() + `/${this.apimodel}/data`, this.formValidate).then(res => {
                this.getData()
                this.modal6 = false;
